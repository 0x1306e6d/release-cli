...
The `project` field SHALL be the only required field in `.release.yaml`. All other fields SHALL have sensible defaults.

#### Scenario: Minimal config
- **WHEN** the config contains only `project: go`
- **THEN** the system operates with default values for all other settings (semver, conventional commits, changelog enabled, GitHub publish enabled)
