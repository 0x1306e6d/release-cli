...
commit:
  ...
  include: [release-metadata.json] # Optional additional release files
...
Set `commit.mode: version-only` to commit only the version manifest and `propagate` targets; changelog generation is skipped. Use `commit.include` to allow specific additional release files, such as metadata generated by a post-bump hook.
